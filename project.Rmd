...
Adrienne Eddy:
Completed the methods section. Worked on introduction. Worked on hypothesis 5, created idea for hypothesis six. worked on code for tests for hypothesis two, six. Gave results for test three, four and six. 


Chad Michael Eyer:
Created and trained on GitHub. Hosted virtual meeting. Created hypotheses. Completed test for hypotheses one, two, and three. Found concrete data set

Valorie Myer:
Created group chat for the group. Completed introduction. Hosted virtual meeting. Wrote conclusions section. Rewrote results as a paragraph.
...
- Do physiochemical attributes significantlty affect quality?
...
I like this, but I feel that it should be in past tense. Is that okay?
-->
...
Provided that the data met the assumptions, we used ANOVA because the two sets of data are independent and this test allows us to compare means.
...
### Physiochemical attributes affecting quality

We used multivariable regression because this allows us to create a model that will show if there is any relationship between any of the attributes and the quality of the wine.  We created pairwise scatterplots which give us a visual idea of what the data looks like.  We ran a lack of fit test to make sure our regression model fits. We ran Pearson's/Spearman's Correlation Coefficient

### Which physiochemical attributes

We performed t-tests on each predictor to determine which, if any, significantly affect quality.

## Citric acid and volatile acidity interaction

We created a regression model to detrmine if there is an interaction between citric acid and volatile acidity. 

### Physiochemical attributes potentially improving quality

We ran t-tests to compare physiochemical attributes between red and white wine to determine the kind of difference.
...
CME: Can we report on mean/median/sd/range/IQR for quality and each of the attributes for red, white, and combined?
-->

This project will be comparing different physiochemical and sensory elements of wine. The wine in this data set are Vinho Verde wines, which comes from Minho region of Portugal. The data was collected between May 2004 and February 2007 and includes 1599 red wine samples and 4898 white wine samples.  
...
wine_fxacid_var.f <- var.test(wine_red$fixed.acidity, wine_white$fixed.acidity, alternative = "t", conf.level = 0.95, ratio = 1)

compare1 <- t.test(wine_white$fixed.acidity, wine_red$fixed.acidity, var.equal = FALSE, alternative = "less")
y <- as.data.frame(wine_red$fixed.acidity)
p1 <- ggplot(y, aes(sample = wine_red$fixed.acidity))
p1 <- p1 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_red$fixed.acidity")+
        theme_bw()

p1

y <- as.data.frame(wine_white$fixed.acidity)
p2 <- ggplot(y, aes(sample = wine_white$fixed.acidity))
p2 <- p2 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_white$fixed.acidity")+
        theme_bw()
...
```{r, include= FALSE}
wine_volacid_var.f <- var.test(wine_red$volatile.acidity, wine_white$volatile.acidity, alternative = "t", conf.level = 0.95, ratio = 1)

compare2 <- t.test(wine_white$volatile.acidity, wine_red$volatile.acidity, var.equal = FALSE, alternative = "less")

y <- as.data.frame(wine_red$volatile.acidity)
p3 <- ggplot(y, aes(sample = wine_red$volatile.acidity))
p3 <- p3 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_red$volatile.acidity")+
        theme_bw()

p3

y <- as.data.frame(wine_white$volatile.acidity)
p4 <- ggplot(y, aes(sample = wine_white$volatile.acidity))
p4 <- p4 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_white$volatile.acidity")+
        theme_bw()
...
```{r, include= FALSE}
wine_resug_var.f <- var.test(wine_red$residual.sugar, wine_white$residual.sugar, alternative = "t", conf.level = 0.95, ratio = 1)

compare3 <- t.test(wine_white$residual.sugar, wine_red$residual.sugar, var.equal = FALSE, alternative = "greater")

y <- as.data.frame(wine_red$residual.sugar)
p5 <- ggplot(y, aes(sample = wine_red$residual.sugar))
p5 <- p5 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_red$residual.sugar")+
        theme_bw()

p5

y <- as.data.frame(wine_white$residual.sugar)
p6 <- ggplot(y, aes(sample = wine_white$residual.sugar))
p6 <- p6 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_white$residual.sugar")+
        theme_bw()
...
```{r, include= FALSE}
wine_fsudi_var.f <- var.test(wine_red$free.sulfur.dioxide, wine_white$free.sulfur.dioxide, alternative = "t", conf.level = 0.95, ratio = 1)

compare4 <- t.test(wine_white$free.sulfur.dioxide, wine_red$free.sulfur.dioxide, var.equal = FALSE, alternative = "greater")


y <- as.data.frame(wine_red$free.sulfur.dioxide)
p7 <- ggplot(y, aes(sample = wine_red$free.sulfur.dioxide))
p7 <- p7 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_red$free.sulfur.dioxide")+
        theme_bw()

p7

y <- as.data.frame(wine_white$free.sulfur.dioxide)
p8 <- ggplot(y, aes(sample = wine_white$free.sulfur.dioxide))
p8 <- p8 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_white$free.sulfur.dioxide")+
        theme_bw()
...
```{r, include= FALSE}
wine_tsudi_var.f <- var.test(wine_red$total.sulfur.dioxide, wine_white$total.sulfur.dioxide, alternative = "t", conf.level = 0.95, ratio = 1)

compare5 <- t.test(wine_white$total.sulfur.dioxide, wine_red$total.sulfur.dioxide, var.equal = FALSE, alternative = "greater")


y <- as.data.frame(wine_red$total.sulfur.dioxide)
p9 <- ggplot(y, aes(sample = wine_red$total.sulfur.dioxide))
p9 <- p9 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_red$free.total.sulfur.dioxide")+
        theme_bw()

p9

y <- as.data.frame(wine_white$total.sulfur.dioxide)
p10 <- ggplot(y, aes(sample = wine_white$total.sulfur.dioxide))
p10 <- p10 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_white$total.sulfur.dioxide")+
        theme_bw()
...
```{r, include= FALSE}
wine_dens_var.f <- var.test(wine_red$density, wine_white$density, alternative = "t", conf.level = 0.95, ratio = 1)

compare6 <- t.test(wine_white$density, wine_red$density, var.equal = FALSE, alternative = "less")


y <- as.data.frame(wine_red$density)
p11 <- ggplot(y, aes(sample = wine_red$density))
p11<- p11 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_red$density")+
        theme_bw()

p11

y <- as.data.frame(wine_white$total.sulfur.dioxide)
p12 <- ggplot(y, aes(sample = wine_white$density))
p12 <- p12 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_white$total.density")+
        theme_bw()
...
```{r, include= FALSE}
wine_ph_var.f <- var.test(wine_red$pH, wine_white$pH, alternative = "t", conf.level = 0.95, ratio = 1)

compare7 <- t.test(wine_white$pH, wine_red$pH, var.equal = TRUE, alternative = "less")

y <- as.data.frame(wine_red$pH)
p13 <- ggplot(y, aes(sample = wine_red$pH))
p13<- p13 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_red$pH")+
        theme_bw()

p13

y <- as.data.frame(wine_white$pH)
p13 <- ggplot(y, aes(sample = wine_white$pH))
p13 <- p13 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_white$total.pH")+
        theme_bw()
...
```{r, include= FALSE}
wine_sulp_var.f <- var.test(wine_red$sulphates, wine_white$sulphates, alternative = "t", conf.level = 0.95, ratio = 1)

compare8 <- t.test(wine_white$sulphates, wine_red$sulphates, var.equal = FALSE, alternative = "less")

y <- as.data.frame(wine_red$sulphates)
p13 <- ggplot(y, aes(sample = wine_red$sulphates))
p13<- p13 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_red$sulphates")+
        theme_bw()

p13

y <- as.data.frame(wine_white$sulphates)
p13 <- ggplot(y, aes(sample = wine_white$sulphates))
p13 <- p13 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_white$total.sulphates")+
        theme_bw()
...
```{r, include= FALSE}
wine_alc_var.f <- var.test(wine_red$alcohol, wine_white$alcohol, alternative = "t", conf.level = 0.95, ratio = 1)

compare8 <- t.test(wine_white$alcohol, wine_red$alcohol, var.equal = FALSE, alternative = "greater")
compare8
y <- as.data.frame(wine_red$alcohol)
p15 <- ggplot(y, aes(sample = wine_red$alcohol))
p15<- p15 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_red$alcohol")+
        theme_bw()



y <- as.data.frame(wine_white$alcohol)
p16 <- ggplot(y, aes(sample = wine_white$alcohol))
p16 <- p16 + 
        stat_qq() + 
        stat_qq_line() +
  labs(title="qq plot wine_white$total.alcohol")+
        theme_bw()
```

The first research question was if the variance of quality was the same for both red and white wines. There is sufficent evidence to suggest that the variability of the quality scores of red and white wines is significantly different, with ($`r p.value.string(wine_quality_var.f$p.value)`$) at the $\alpha = .05$ level. Because there is a difference in variablity, it impacted which tests were run in the following hypotheses. 


The next research question was to determine in the mean quality of the two types of wines were different. There is sufficent evidence to suggest that the mean quality is different, with ($`r p.value.string(wine_anova_table[[1]][["Pr(>F)"]][1])`$) at the $\alpha = .05$ level. 

The natural follow-up question is, "Which wine has a greater mean quality" From several t-tests, it was determined that there is sufficient evidence to suggest that the mean quality of red wine is less than the mean quality of white wine at the $\alpha=0.05$ level ($`r p.value.string(winequal2$p.value[1])`$). 

The next research question wsa to see what physiochemical properties affect the quality of wine and which properties white wine has more or less of compared to red wine. 

Several physiochemical properties play a significant role in affecting quality of wine at the $\alpha=0.05$ level.

The multivariable ANOVA test shows that wine quality scores are significantly affected by fixed acidity, volatily acidity, residual sugar, free sulfur dioxide, total sulfur dioxide, density, pH, sulphates, and alcohol. Quality scores are not dependent on citric acid or chlorides.
Knowing that these are all significant predictors of quality, it would be interesting to see whether white wine had more or less of these properties.


Winemakers use citric acid to sterilize equipment between batches. Citric acid is also found in fruitier wines.  However, citric acid can contribute to volatile acidity which leads to wine spoilage. Even though citric acid is not a significant predictor, we want to look to see if there is an interaction between citric acid and volatile acidity.


$`r p.value.string(wine_red_table[[1]][["Pr(>F)"]][3])`$.


There is sufficient evidence to suggest that there is an interaction between citric acid and volatile acidity because $`r p.value.string(wine_red_table[[1]][["Pr(>F)"]][3])`$.




White wine has less fixed acidity, less volatile acidity (spoilage), more residual sugar, greater free sulfur dioxide, greater total sulfur dioxide, less dense, lower pH, fewer sulphates, and more alcohol.
