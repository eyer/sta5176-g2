...
$\hat{y} = `r round(h3_model.coef[[1]], digits = 2)` + `r round(h3_model.coef[[2]], digits = 2)`$ fixed acidity  $`r round(h3_model.coef[[3]], digits = 2)`$ volatile acidity $`r round(h3_model.coef[[4]], digits = 2)`$ citric acid $+ `r round(h3_model.coef[[5]], digits = 2)`$ residual sugar $`r round(h3_model.coef[[6]], digits = 2)`$ chlorides $+ `r round(h3_model.coef[[7]], digits = 2)`$ free sulfur dioxide $+ `r round(h3_model.coef[[8]], digits = 4)`$ total sulfur dioxide $`r round(h3_model.coef[[9]], digits = 2)`$ density $+ `r round(h3_model.coef[[10]], digits = 2)`$ pH $+ `r round(h3_model.coef[[11]], digits = 2)`$ sulphates $+ `r round(h3_model.coef[[12]], digits = 2)`$ alcohol
...
```
<br>
When we look at fixed acidity, we see a slope of $`r round(h3_model.coef[[2]], digits = 2)`$.  This means for every 1 unit change of fixed acidity, we can expect to see a $`r round(h3_model.coef[[2]], digits = 2)`$ change in quality.  Or, for every $`r round(1/h3_model.coef[[2]], digits = 2)`$ units change in fixed acidity, we can expect a 1 unit change in quality.
<br>
```{r out.width = '40%'}
wine %>% 
...
```
<br>
When we look at volatile acidity, we see a slope of $`r round(h3_model.coef[[3]], digits = 2)`$.  This means for every 1 unit change of volatile acidity, we can expect to see a $`r round(h3_model.coef[[3]], digits = 2)`$ change in quality.  Or, for every $`r round(1/h3_model.coef[[3]], digits = 2)`$ units change in volatile acidity we can expect a 1 unit change in quality.
<br>
```{r out.width = '40%'}
wine %>% 
...
```
<br>
When we look at citric acid, we see a slope of $`r round(h3_model.coef[[4]], digits = 2)`$.  This means for every 1 unit change of citric acid, we can expect to see a $`r round(h3_model.coef[[4]], digits = 2)`$ change in quality.  Or, for every $`r round(1/h3_model.coef[[4]], digits = 2)`$ units change in citric acid, we can expect a 1 unit change in quality.
<br>
```{r out.width = '40%'}
wine %>% 
...
```
<br>
When we look at residual sugar, we see a slope of $`r round(h3_model.coef[[5]], digits = 2)`$.  This means for every 1 unit change of residual sugar, we can expect to see a $`r round(h3_model.coef[[5]], digits = 2)`$ change in quality.  Or, for every $`r round(1/h3_model.coef[[5]], digits = 2)`$ units change in residual sugar, we can expect a 1 unit change in quality.
<br>
```{r out.width = '40%'}
wine %>% 
...
```
<br>
When we look at chlorides, we see a slope of $`r round(h3_model.coef[[6]], digits = 2)`$.  This means for every 1 unit change of chlorides, we can expect to see a $`r round(h3_model.coef[[6]], digits = 2)`$ change in quality.  Or, for every $`r round(1/h3_model.coef[[6]], digits = 2)`$ units change in chlorides, we can expect a 1 unit change in quality.
<br>
```{r out.width = '40%'}
wine %>% 
...
```
<br>
When we look at free sulfur dioxide, we see a slope of $`r round(h3_model.coef[[7]], digits = 2)`$.  This means for every 1 unit change of free sulfur dioxide, we can expect to see a $`r round(h3_model.coef[[7]], digits = 2)`$ change in quality.  Or, for every $`r round(1/h3_model.coef[[7]], digits = 2)`$ units change in free sulfur dioxide, we can expect a 1 unit change in quality.
<br>
```{r out.width = '40%'}
wine %>% 
...
```
<br>
When we look at density, we see a slope of $`r round(h3_model.coef[[9]], digits = 2)`$.  This means for every 1 unit change of density, we can expect to see a $`r round(h3_model.coef[[9]], digits = 2)`$ change in quality.  Or, for every $`r round(1/h3_model.coef[[9]], digits = 2)`$ units change in density, we can expect a 1 unit change in quality.
<br>
```{r out.width = '40%'}
wine %>% 
...
```
<br>
When we look at pH, we see a slope of $`r round(h3_model.coef[[10]], digits = 2)`$.  This means for every 1 unit change of pH, we can expect to see a $`r round(h3_model.coef[[10]], digits = 2)`$ change in quality.  Or, for every $`r round(1/h3_model.coef[[10]], digits = 2)`$ units change in pH, we can expect a 1 unit change in quality.
<br>
```{r out.width = '40%'}
wine %>% 
...
```
<br>
When we look at sulphates, we see a slope of $`r round(h3_model.coef[[11]], digits = 2)`$.  This means for every 1 unit change of sulphates, we can expect to see a $`r round(h3_model.coef[[11]], digits = 2)`$ change in quality.  Or, for every $`r round(1/h3_model.coef[[11]], digits = 2)`$ units change in sulphates, we can expect a 1 unit change in quality.
<br>
```{r out.width = '40%'}
wine %>% 
...
```
<br>
When we look at alcohol, we see a slope of $`r round(h3_model.coef[[12]], digits = 2)`$.  This means for every 1 unit change of alcohol, we can expect to see a $`r round(h3_model.coef[[12]], digits = 2)`$ change in quality.  Or, for every $`r round(1/h3_model.coef[[12]], digits = 2)`$ units change in alcohol, we can expect a 1 unit change in quality.
<br>
